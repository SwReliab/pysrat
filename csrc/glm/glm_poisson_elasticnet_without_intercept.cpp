...
//
// Poisson GLM with Elastic Net penalties
...
//      lambda_l2[j] >= 0
//      penalty = sum_j lambda_l1[j] * |beta_j|
...
// - no intercept parameter is fitted
//
// Parameterization:
...
// 4) vector-L2 overload uses fast per-coordinate CD updates
// 5) matrix-L2 overload supports correlated shrinkage using cached
//    g = lambda_l2_mat * beta during coordinate descent
...
GLMPoissonENetResult glm_poisson_elasticnet_without_intercept_core(
  ...
  auto penalized_obj = [&](const Eigen::VectorXd& beta_try) -> double {
    ...
    double pen_l1 = 0.0;
    for (int j = 0; j < p; ++j) {
      pen_l1 += lambda_l1[j] * std::abs(beta_try[j]);
    }
  ...
  for (int outer = 0; outer < max_iter; ++outer) {
    ...
    // ---- inner: coordinate descent on penalized WLS without intercept ----
    // residual r = ytilde - Xs*beta
    Eigen::VectorXd r = ytilde - (Xs * beta);
    ...
    // g = lambda_l2_mat * beta_new
    Eigen::VectorXd g = lambda_l2_mat * beta_new;
    ...
    for (int cd_it = 0; cd_it < cd_max_iter; ++cd_it) {
      ...
      for (int j = 0; j < p; ++j) {
        ...
        // For correlated L2:
        // rho_j = x_j^T(Wr) + (x_j^T W x_j)*beta_j - sum_{k!=j} Lambda_{jk} beta_k
        //
        // Since g = Lambda * beta, we have
        //   sum_{k!=j} Lambda_{jk} beta_k = g[j] - Lambda_jj * beta_j
        const double coupling_excl_j = g[j] - lambda_l2_mat(j, j) * bj_old;

        const double rho = xTu + aj_x[j] * bj_old - coupling_excl_j;
        const double bj_new = soft_threshold(rho, lambda_l1[j]) / aj[j];
        const double delta = bj_new - bj_old;
        ...
        if (delta != 0.0) {
          ...
          beta_new[j] = bj_new;
          g.noalias() += lambda_l2_mat.col(j) * delta;
        }
