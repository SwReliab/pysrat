...
class TruncatedExtremeValueMinNHPP(NHPPModel):
    name = "txvmin"
    df = 3
    ...
    def init_params(self, data: NHPPData) -> np.ndarray:
        return np.array([data.total, -data.mean, data.max / 3.0], dtype=float)

    def em_step(self, params: np.ndarray, data: NHPPData, **kwargs) -> dict:
        core = data.to_core_dict()
        eres = _core.em_txvmin_estep(np.asarray(params, dtype=float), core)
        method = kwargs.get("method", "L-BFGS-B")
        options = kwargs.get("options")
        loc_scale = optimize_params(
            _core.em_txvmin_pllf,
            core,
    ...
    def mvf(self, t):
        ...
        return self.omega_() * ptgumbel(t, loc=self.loc_(), scale=self.scale_(), lower_tail=True, min=True)

    def intensity(self, t):
        t = np.asarray(t, dtype=float)
        return self.omega_() * dtgumbel(t, loc=self.loc_(), scale=self.scale_(), log=False, min=True)
